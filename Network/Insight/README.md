...
The Minion image (`repository`, `tag`, `digest`, and `pullPolicy`) and
`nodeSelector` are values-driven. Home1 explicitly selects
`core.resolvemy.host/x86-64-v3=true`; DC1 intentionally leaves `nodeSelector`
empty and uses the pinned `opennms/minion:35.0.0` image.
