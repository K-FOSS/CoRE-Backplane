...
The Sonarr (`Media/TV`), Radarr (`Media/Movies`), SABnzbd (`Media/NZB`), and
Stash (`Media/Streaming`) public routes are protected by Authentik
single-application forward auth. Their owning ApplicationSets are
`Apps/Media/TV.yaml`, `Apps/Media/Movies.yaml`, `Apps/Media/NZB.yaml`, and
`Apps/Media/Streaming.yaml`; all currently select the
`core-home1-talos-prod` bare metal infrastructure cluster and deploy into
`core-media`.
...
entry. Sonarr, Radarr, and SABnzbd grant access to `Media Consumers`; Stash
grants access only to `Private Stash`. The proxy's external host is the
corresponding `tv`, `movies`, `nzb`, or `accessmyporn.download` public
hostname.
...
and invalidation flows named in the modules, the `Media Consumers` and
`Private Stash` groups, the shared proxy service, the accepted shared
`/outpost.goauthentik.io` route, and the public Gateway listeners. No Authentik
token or application secret is stored or published by these charts.
...
entitlement, and expected access-group bindings. Then verify the HTTPRoute is
`Accepted`, the SecurityPolicy is attached without resolution errors, an
unauthenticated browser is redirected to Authentik, a member of the configured
group can load the application, and an unauthorized account is denied. For
Stash, explicitly verify that a `Media Consumers` member without `Private
Stash` membership is denied. Application pod readiness alone does not validate
the access path.
